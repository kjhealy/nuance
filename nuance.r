...
### Generates keyword data frame for a single
### keyword in a given unit
proc.unit <- function(unit="Sociology",
                      subdir="terms",
                      keyword="nuance", ...){
...
### Fix for AJS misclassifications
    fix.ajs <- function(x, ...){
        ...
        tab.bookreviews <- ajs.df %>% group_by(year) %>%
            filter(length < 4) %>%
            group_by(year) %>%
            tally() %>%
...
### raw pagerange column to df with start and end pages as numbers
### fixes page numbering inconsistency
clean.pages <- function(raw.pages){
    ...
    ## AJS is sometimes inconsistent with page ranges
    ## e.g. saying 1125-27 instead of 1125-1127
...
### Apply proc.unit() to a vector of unit names
### for a given keyword, clean it up,
### and calculate the relative rate for the keyword
make.keyword.df <- function(unit.names,
                            full.names=NULL,
                            new.labels=FALSE,
                            keyword="nuance",
                            subdir = "manual", ...){
...
### Reorder for labeling purposes
    unit.tab <- data %>% group_by(unit) %>%
        filter(year>2001) %>%
        summarize(Med = median(rate)) %>% ungroup() %>%
        arrange(desc(Med))

    data$unit <- factor(data$unit,
                        levels = unit.tab$unit,
                        ordered = TRUE)
    ...
    colnames(data.total) <- c("year", "total.articles", paste0("total.", keyword), "total.rate", "total")
...
fields <- list("Sociology", "Economics", "Politics",
               "Anthropology", "History", "Philosophy", "Psychology")

data.fields <- make.keyword.df(unit.names = fields,
                               subdir = "terms")

### Basic field-level rates
### Too many fields for the shape aesthetic

pdf(file="figures/nuance-rate-by-discipline.pdf", width=9, height=8)
p <- ggplot(subset(data.fields, year<2014 & year>1860),
            aes(x=year, y=rate, color=unit,
                fill=unit))

p + geom_point(alpha=0.7) + geom_smooth(se=FALSE) +
    scale_y_continuous(labels = scales::percent) +
    theme(legend.position = "right") +
    labs(x = "Year",
         y = "Journal Articles mentioning 'Nuance' or 'Nuanced'",
         fill = "Discipline",
         color = "Discipline") +
    ggtitle("Rates of Nuance Across the Disciplines, 1860-2013")
credit("Data: JSTOR.")
dev.off()
...
linelab.df <- data.frame(unit=c("Economics", "Psychology",
                                 "Philosophy", "Politics",
                                 "History", "Sociology", "Anthropology"),
                         y.loess=c(-1, 1.1, 2.65, 3.75, 4.4, 4.7, 5.75),
                         y.gam=c(-0.8, 0.9, 2.7, 3.25, 4.2, 4.5, 5.1))



pdf(file="figures/nuance-rate-by-discipline-relative.pdf",
    width=9, height=8.2, pointsize = 10)
p <- ggplot(subset(data.fields, year<2014 & year>1860),
            aes(x=year, y=relative.keyword*100,
                color=unit,
                fill=unit))

p1 <- p + geom_hline(yintercept = 0, color = "gray70") +
    geom_point(size=0.5) + geom_smooth(se=FALSE) +
    geom_text(data=linelab.df,
              aes(x=2014, y=y.loess, label=unit),
              hjust=0,
              size=3) +
    theme(legend.position = "right") +
    labs(x = "Year",
         y = "Percentage Point Difference from Base Rate",
         fill = "Discipline",
         color = "Discipline") +
    xlim(1860, 2020) +
    ylim(-1.5, 7.5) +
    guides(fill=FALSE, color=FALSE) +
    ggtitle("Nuance Across the Disciplines, 1860-2013")

## Turn off the clipping so the labels are legible
gt <- ggplot_gtable(ggplot_build(p1))
gt$layout$clip[gt$layout$name=="panel"] <- "off"
grid.draw(gt)
credit("Kieran Healy. Data: JSTOR. Base rate is percent mentions across all research articles in the JSTOR corpus.", size=0.5)
dev.off()


pdf(file="figures/nuance-rate-by-discipline-relative-SE-jit-gam.pdf",
    width=9, height=9, pointsize = 10)
p <- ggplot(subset(data.fields, year<2014 & year>1860),
            aes(x=year, y=relative.keyword*100,
                color=unit,
                fill=unit))
...
p1 <- p + geom_hline(yintercept = 0, color = "gray20") +
    ...
    geom_text(data=linelab.df,
              aes(x=2014, y=y.gam, label=unit),
              hjust=0,
              size=3) +
    theme(legend.position = "right") +
    labs(x = "Year",
         y = "Percentage Points Difference from Base Rate",
         fill = "Discipline",
         color = "Discipline") +
    xlim(1860, 2020) +
    guides(fill=FALSE, color=FALSE) +
...
jl.names <- c("Amer. Economic Rev.",
              "Amer. Sociological Rev.",
              "Amer. Jl of Political Science",
              "Amer. Political Science Rev.",
              "Amer. Jl Sociology",
              "Demography",
              "Jl of Politics",
              "Jl of Political Economy",
              "Quarterly Jl of Economics",
              "Social Forces")

data.journals <- make.keyword.df(unit.names = journals,
                                 full.names = jl.names,
                                 subdir = "terms",
                                 new.labels = TRUE)

pdf(file="figures/nuance-rate-by-journal-relative-facet.pdf",
    width=10, height=5, pointsize = 10)
p <- ggplot(subset(data.journals, year<2014 & year>1899),
            aes(x=year, y=relative.keyword*100))

p1 <- p + geom_hline(yintercept = 0, color = "gray20") +
    geom_point(size=0.5, alpha=0.5) +
    geom_smooth(method="gam", formula = y ~ ns(x, 3)) +
    facet_wrap(~ longlab, ncol = 5) +
    labs(x = "",
         y = "Percentage Point Difference from Base Rate") +
    ...
    ggtitle("Nuance in Eight Social Science Journals, 1900-2013")
...
data.socjournals <- make.keyword.df(soc.journals,
                                    full.names=soc.names,
                                    subdir = "terms",
                                    new.labels = TRUE)

pdf(file="figures/nuance-rate-by-soc-journal-relative-facet.pdf",
    width=9, height=3, pointsize = 10)
p <- ggplot(subset(data.socjournals, year<2014 & year>1949),
            aes(x=year, y=relative.keyword*100))
...
p1 <- p + geom_hline(yintercept = 0, color = "gray20") +
    ...
    theme(axis.title.y=element_text(size = rel(0.8)),
          ...
          axis.text.x=element_text(size=rel(0.6))) +
    ggtitle("Nuance in Five Sociology Journals, 1950-2013")
...
soc.alljournals <- c("ASQ", "ASR", "AJS", "Demography",
                     "JHSB", "JMF", "GS", "SF", "SM", "SPQ",
                     "ST", "TS")

soc.names <- c("Administrative Sci. Q.", "ASR", "AJS", "Demography",
                "JHSB", "J. Marriage & Family", "Gender and Society",
                "Social Forces", "Soc. Methodology", "Soc. Psych. Q.",
                "Soc. Theory", "Theory and Society")


data.allsocjournals <- make.keyword.df(soc.alljournals,
                                      full.names = soc.names,
                                      new.labels = TRUE,
                                      subdir = "terms")

pdf(file="figures/nuance-rate-by-allsoc-journal-relative-facet.pdf",
    width=12, height=6, pointsize = 10)

p <- ggplot(subset(data.allsocjournals, year<2014 & year>1959),
            aes(x=year, y=relative.keyword*100))
...
soc.names <- c("American Sociological Review",
               "American Journal of Sociology",
               "Social Forces")

data.socjournals <- make.keyword.df(soc.journals,
                                    keyword = "nuance",
                                    full.names=soc.names, new.labels = TRUE)

pdf(file="figures/nuance-rate-relative.pdf", width=7, height=7)

p <- ggplot(subset(data.socjournals, year<2013),
            aes(x=year, y=relative.keyword, color=longlab,
                fill=longlab,
                shape=longlab))

p1 <- p + geom_jitter() + geom_smooth(se=FALSE) +
    scale_y_continuous(labels = scales::percent) +
...
data.nuance <- make.keyword.df(soc.journals,
                            full.names = soc.names,
                            new.labels = TRUE,
                            subdir = "terms",
                            keyword = "nuance")
...
data.soph <- make.keyword.df(soc.journals,
                            full.names = soc.names,
                            new.labels = TRUE,
                            subdir = "terms",
...
data.subtle <- make.keyword.df(soc.journals,
                            full.names = soc.names,
                            new.labels = TRUE,
                            subdir = "terms",
                            keyword = "subtle")

data.subtlety <- make.keyword.df(soc.journals,
                            full.names = soc.names,
                            new.labels = TRUE,
                            subdir = "terms",
                            keyword = "subtlety")
...
p <- ggplot(subset(data.soph, year<2013),
            aes(x=year, y=rate, color=longlab,
                fill=longlab,
                shape=longlab))

pdf(file="figures/sophisticated-rate.pdf", width=7, height=7)
p + geom_jitter() + geom_smooth(se=FALSE) +
    scale_y_continuous(labels = scales::percent) +
    theme(legend.position = "bottom",
          plot.title=element_text(size=rel(0.8))) +
    labs(x = "",
         y = "Articles mentioning 'Sophisticated'",
         fill = "Journal",
         shape = "Journal",
         color = "Journal") +
    scale_color_manual(values = my.colors("bly")) +
    ggtitle("'Sophisticated'")
    credit("Data: JSTOR.")
dev.off()


p <- ggplot(subset(data.subtle, year<2013),
            aes(x=year, y=rate, color=longlab,
                fill=longlab,
                shape=longlab))
...
p <- ggplot(subset(data.subtlety, year<2013),
            aes(x=year, y=rate, color=longlab,
                fill=longlab,
                shape=longlab))
...
p + geom_jitter() + geom_smooth(se=FALSE) +
    ...
    theme(legend.position = "bottom",
          plot.title=element_text(size=rel(0.8))) +
    labs(x = "",
         y = "Articles mentioning 'Subtlety'",
         fill = "Journal",
         shape = "Journal",
         color = "Journal") +
    scale_color_manual(values = my.colors("bly")) +
...
p <- ggplot(subset(data.nuance, year < 2013),
            aes(x = year, y = rate, color=longlab,
                fill=longlab,
                shape=longlab))
...
p + geom_jitter() + geom_smooth(se=FALSE) +
    ...
    labs(x = "",
         y = "Articles mentioning 'Nuance' or 'Nuanced'",
         fill = "Journal",
         shape = "Journal",
         color = "Journal") +
    scale_color_manual(values = my.colors("bly"))
